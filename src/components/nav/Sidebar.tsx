    ...
    <aside className="hidden w-64 shrink-0 flex-col border-r border-line bg-paper lg:flex">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-3">
            ...
            <div key={group.label} className="mb-1">
              <button
                ...
                className={cn(
                  ...
                  groupActive ? "text-brand" : "text-ink-muted hover:text-ink",
                )}
              >
                      ...
                      <li key={item.href}>
                        <Link
                          ...
                          className={cn(
                            ...
                            active ? "bg-brand-soft font-medium text-brand" : "text-ink-muted hover:bg-paper-sunken hover:text-ink",
                          )}
                        >
                          {item.label}
                        </Link>
        ...
        {isPlatformAdmin && (
          <Link
            href="/admin"
            className="mt-3 flex items-center gap-2 rounded-md bg-paper-sunken px-2 py-1.5 text-sm font-medium text-ink-muted hover:text-ink"
          >
